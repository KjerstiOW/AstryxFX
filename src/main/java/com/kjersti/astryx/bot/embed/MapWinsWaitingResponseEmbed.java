...
public class MapWinsWaitingResponseEmbed {
    public static final String EMBED_LANGUAGE_LOCATION = "bot.embed.";
    private final EmbedCreateSpec.Builder builder;

    public MapWinsWaitingResponseEmbed(String teamId, String teamName) {
        String footer = LanguageLoader.get(EMBED_LANGUAGE_LOCATION + "footer");
        String title = LanguageLoader.get(EMBED_LANGUAGE_LOCATION + "map_wins_fetching_response.title")
                .replace("{team_id}", teamId)
                .replace("{team_name}", teamName);
        String desc = LanguageLoader.get(EMBED_LANGUAGE_LOCATION + "map_wins_fetching_response.desc")
                .replace("{team_id}", teamId)
                .replace("{team_name}", teamName);
