...
@BotCommand(
        id="team_vetoes",
        desc="Gets team vetoes"
)
public class TeamVetoCommand implements BaseCommand {
    ...
    public Mono<Void> run(GatewayDiscordClient client, ApplicationCommandInteractionEvent event) {
        ...
        InputStream chart = PythonRunner.createMapVetoPythonChart(teamId, teamName, startDate, endDate);
        ...
        PythonRunner.deleteChart(PythonRunner.MAP_VETO_CHART_PATH);
    ...
    public void onPythonError(ApplicationCommandInteractionEvent event, String teamId, String teamName) {
        String logFilePath = PythonRunner.VETOES_LOG_PATH;

        PythonErrorEmbed embed = new PythonErrorEmbed(teamId, teamName, "Veto", logFilePath);
